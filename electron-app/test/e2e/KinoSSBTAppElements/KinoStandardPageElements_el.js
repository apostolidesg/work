...
describe('TC KinoStandardPageElements_el.js: Check elements of Kino Bet Page', function () {
  ...
  //     this.app.client.getText(elements.KinoScanError).should.eventually.be.equal('ΣΦΑΛΜΑ ΣΥΣΚΕΥΗΣ ΣΑΡΩΣΗΣ'),
  //   ])
  // })
  ...
  it('Check T&C element and close with X', function () {
    let self = this;
    return Promise.all([
      this.app.client.getText(elements.KinoTCTitle).should.eventually.be.equal('ΟΡΟΙ ΣΥΜΜΕΤΟΧΗΣ'),
      ...
      this.app.client.isVisible(elements.KINOTCClose).should.eventually.be.true,
      this.app.client.getText(elements.KINOTCOk).should.eventually.be.equal('Εντάξει'),
  ...
  it('Check SSBT balance', function () {
    return this.app.client.getText(elements.KinoBalanceHide).should.eventually.be.equal('Εμφάνιση Υπολοίπου');
  });
  ...
  it('Check Quick Pick field', function () {
    return Promise.all([
      this.app.client.getText(elements.KinoQuickPickLbl).should.eventually.be.equal('ΤΥΧΑΙΑ ΕΠΙΛΟΓΗ'),
  ...
  it('Check Clear button', function () {
    return Promise.all([
      ...
      this.app.client.getText(elements.KinoBetClearAreaLbl).should.eventually.be.equal('ΚΑΘΑΡΙΣΜΟΣ'),
    ]);
  });
  ...
  it('Check consecutive draws fields', function () {
    return Promise.all([
      ...
      this.app.client.getText(elements.KinoBetConsDrawsLbl).should.eventually.be.equal('ΣΥΝΕΧΟΜΕΝΕΣ ΚΛΗΡΩΣΕΙΣ'),
  ...
  it('Confirm exit KINO game', function () {
    ...
    // let self=this;
    // return Promise.all
    //     ([this.app.client.elements(elements.KinoModalAMsgLbl).getAttribute('innerText').should.eventually.be.equal('RETURN TO MAIN MENU'),
    //       this.app.client.elements(elements.KinoModalAMsg).getAttribute('innerText').should.eventually.be.equal('Returning to the Main Menu will cause your current selections to be lost, do you want to continue?'),
    //       this.app.client.elements(elements.KinoModalAYesBtn).getAttribute('innerText').should.eventually.be.equal('Yes'),
    //       this.app.client.elements(elements.KinoModalANoBtn).getAttribute('innerText').should.eventually.be.equal('No')])
    //        .then(function(values){
