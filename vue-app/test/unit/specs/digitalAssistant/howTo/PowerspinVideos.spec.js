...
import PowerspinVideos from '../../../../../src/components/digitalAssistant/howTo/PowerspinVideos.vue';
...
describe('PowerspinVideos.vue', () => {
  let localVue;
  let store;

  const factory = (opts = {}) =>
    mount(PowerspinVideos, {
      localVue,
      store,
      stubs: {
  ...
  it('passes the correct asset-key to AssetWrapper', async () => {
    ...
    expect(asset.props('assetKey')).to.equal('HOW_TO_POWERSPIN_1');
    ...
    expect(asset.props('assetKey')).to.equal('HOW_TO_POWERSPIN_2');
  });

  // NEW TESTS: Test the new changes - Banners component and currentGame computed
  describe('new changes', () => {
    it('computes currentGame as POWERSPIN theme', () => {
      const wrapper = factory();
      expect(wrapper.vm.currentGame).to.equal(Constants.THEMES.POWERSPIN);
    });
    ...
    it('shows Banners component when in portrait mode', () => {
      ...
      expect(banners.props('game')).to.equal(Constants.THEMES.POWERSPIN);
    });
