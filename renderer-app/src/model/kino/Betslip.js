...
import ErrorHandlerService from '../../util/handler/kino/ErrorHandlerService';
...
export default class Betslip {
  constructor(wagerModel) {
    this.bet_areas = [];
    this.bet_areas.push(new Board(1));
    ...
    if (wagerModel) {
      const wagerBoards = wagerModel?.boards || [];
      const betAreasFromModel = wagerBoards.filter((board) => NUMBER_BET_TYPES.includes(board.betType));
      const oddEvenGameFromModel = wagerBoards.find(
        (board) =>
          board.betType === Constants.ILOT_GAMETYPES.ODD ||
          board.betType === Constants.ILOT_GAMETYPES.EVEN ||
          board.betType === Constants.ILOT_GAMETYPES.DRAW
      );
      const columnsGameFromModel = wagerBoards.find((board) => board.betType === Constants.ILOT_GAMETYPES.COLUMNS);

      this.bet_areas = betAreasFromModel.length
        ? betAreasFromModel.map((board, i) => new Board(i + 1, board))
        : this.bet_areas;

      this.consecutiveDraws = wagerModel?.participatingDraws?.multipleDraws || 1;
  ...
  addNewBet() {
    if (this.bet_areas.length < 6) this.bet_areas.push(new Board());
  }
  ...
  calculateValue() {
    let totalValue = 0.0;
    for (let area in this.bet_areas) totalValue += this.bet_areas[area].value;

    this.value =
      (totalValue + this.oddEvenGame.calculateValue() + this.columnsGame.calculateValue()) * this.consecutiveDraws;
  ...
  clearBetslip() {
    this.bet_areas = [];
    for (let i = 1; i < 7; i++) {
      this.bet_areas.push(new Board(i));
    }
    this.consecutiveDraws = 1;
    this.oddEvenGame = new OddEven();
    this.columnsGame = new Columns();
  ...
  ilotFormat() {
    ...
    if (this.columnsGame.isFilled())
      ilotFormattedBetslip.wager.boards.push({
        ...
        multipliers:
          this.columnsGame.columnsAmount.reduce(function (acc, val) {
            return acc + val;
          }) * 2,
        panels: [
          {
  ...
  isFilled() {
    return (
      this.betAreasFilled() || this.oddEvenGame.isFilled() || this.columnsGame.isFilled() || this.consecutiveDraws !== 1
    );
  }
