...
export default class Betslip {
  ...
  addBoard() {
    if (this.wager.boards.length < FireblazeConstants.MAX_BOARDS) {
      const existingNumbers = new Set(this.wager.boards.map((board) => board.betType));
      const nextBetType = Object.values(FireblazeConstants.BET_TYPES).find((betType) => !existingNumbers.has(betType));
