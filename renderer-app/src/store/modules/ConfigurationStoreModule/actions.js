...
import { fetchCmsConfiguration, getElectronEnv, loadConfiguration, loadElectronAppVersion } from '@/util/configLoader';
...
const actions = {
  ...
  async [types.actions.INIT_CONFIGURATION]({ dispatch }, { electronEnv } = {}) {
    const resolvedElectronEnv = electronEnv || (await getElectronEnv());
    const { configuration, voucher } = await loadConfiguration({
      electronEnv: resolvedElectronEnv,
    });

    const cmsConfiguration = await fetchCmsConfiguration();
    const mergedConfiguration = merge({}, cmsConfiguration || {}, configuration || {});

    await dispatch(types.actions.SET_CONFIGURATION, { configuration: mergedConfiguration });

    if (voucher) {
      await dispatch(types.actions.SET_VOUCHER, { voucher });
    }

    const electronAppVersion = await loadElectronAppVersion();
    if (electronAppVersion) {
      const updatedConfiguration = merge({}, mergedConfiguration, { ELECTRON_APP_VERSION: electronAppVersion });
      await dispatch(types.actions.SET_CONFIGURATION, { configuration: updatedConfiguration });
    }
  ...
  [types.actions.LOAD_FAQ_BOX]: async ({ getters, commit }, { pageType, boxId, defaultContent = {} }) => {
    ...
    let result;
    if (hit) {
      result = { title: hit.title, content: hit?.allwynDescription || hit.description };
    } else {
      const fb = (defaultContent[pageType] || []).find((b) => b.route?.includes(boxId));
      result = fb ? { title: fb.title, content: fb?.allwynDescription || fb.description } : { title: '', content: '' };
    }
