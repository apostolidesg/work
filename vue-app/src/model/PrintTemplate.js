...
import Betslip from './powerspin/Betslip';
...
import opapVerticalLogo from '../assets/logo_vertical.jpeg';
import opapHorizontalLogo from '../assets/logo2.png';
...
export default class PrintTemplate {
  ...
  async prepareTemplate(printType, data, terminalId, locale) {
    this.setIsL10Version(data.serialNumbers);
    const renderedHTML = await this.renderHTML(printType, data, terminalId, locale);
    ...
    return rasterizeHTML.drawHTML(renderedHTML, betSlipCanvas).then((renderResult) => {
      const idata = betSlipCanvasContext.getImageData(0, 0, Constants.RECEIPT_WIDTH, betSlipHeight);
      return {
        imageData: idata,
  ...
  async processData(printType, data, terminalId, locale) {
    if (data.companyInfo) {
    ...
    if (printType === Constants.PRINT_TYPE.POWERSPIN_BETSLIP) {
      return this.processPowerpinBetslipData(data, locale);
    }

    if (printType === Constants.PRINT_TYPE.EUROJACKPOT_BETSLIP) {
      return this.processEurojackpotBetslipData(data, locale);
    }

    if (printType === Constants.PRINT_TYPE.FIREBLAZE_BETSLIP) {
      return this.processFireblazeBetslipData(data, locale);
    }

    if (printType === Constants.PRINT_TYPE.INSTANT_WIN_VOUCHER) {
      return this.processInstantWinData(data, locale);
    }
    ...
    switch (printType) {
      case Constants.PRINT_TYPE.BETSLIP:
        ...
        data.logoUrl = this.getLogoUrl(data.brandName, orientation);
      ...
      case Constants.PRINT_TYPE.VOUCHER:
        ...
        data.logoUrl = this.getLogoUrl(data.brandName, Constants.PRINT_LOGO_ORIENTATION.HORIZONTAL);
        ...
        data.currentDate = moment().format('DD.MM.YYYY HH:mm:ss');
        break;
    }
  ...
  async processPowerpinBetslipData(initialData, locale) {
    ...
    data.wager.participatingDraws.firstDrawTime = moment(parseInt(data.wager.participatingDraws.firstDrawTime)).format(
      'DD/MM/YYYY HH:mm:ss'
    );
    data.wager.participatingDraws.lastDrawTime = moment(parseInt(data.wager.participatingDraws.lastDrawTime)).format(
      'DD/MM/YYYY HH:mm:ss'
    );

    data.currentDate = moment().format('DD/MM/YY');
    data.currentDateTime = moment().format('DD/MM/YY HH:mm:ss');

    const { serialNumbers, iSecure } = data;
    const serialNumber = this.getSerialNumber(serialNumbers[0], iSecure);
    data.logoUrl = this.getLogoUrl(data.brandName, Constants.PRINT_LOGO_ORIENTATION.VERTICAL);
    ...
    data.markets = Object.values(betslip.wager.markets.categories)
      .filter((category) => !betslipUtils.isMarketsCategoryEmpty(category))
      .map((category) => {
  ...
  async processEurojackpotBetslipData(initialData, locale) {
    const data = { ...initialData };
    data.wager.cost = Utilities.formatNumber(data.wager.cost);

    data.wager.participatingDraws.firstDrawTime = moment(
      parseInt(data.wager.participatingDraws.firstDrawTime) * 1000
    ).format('DD/MM/YYYY HH:mm:ss');
    data.wager.participatingDraws.lastDrawTime = moment(
  ...
  async processFireblazeBetslipData(initialData, locale) {
    const data = { ...initialData };
  ...
  async processInstantWinData(initialData, locale) {
    const data = { ...initialData };
  ...
  async renderHTML(printType, data, terminalId, locale) {
    switch (printType) {
      case Constants.PRINT_TYPE.BETSLIP:
        const betslipTemplate = this.getBetslipTemplate(locale);
        return Mustache.render(betslipTemplate, await this.processData(printType, data, terminalId, locale));
      case Constants.PRINT_TYPE.POWERSPIN_BETSLIP:
        return Mustache.render(
          locale === 'el' ? powerspinBetslipTemplateEl : powerspinBetslipTemplateEn,
          await this.processData(printType, data, terminalId, locale)
        );
  ...
  getLogoUrl(brandName, orientation) {
    const isAllwyn = brandName === Constants.BRAND_NAMES.ALLWYN;
    const isVertical = orientation === Constants.PRINT_LOGO_ORIENTATION.VERTICAL;

    if (isVertical) return isAllwyn ? allwynVerticalLogo : opapVerticalLogo;
    return isAllwyn ? allwynHorizontalLogo : opapHorizontalLogo;
  }
  ...
  calculatePowerspinTemplateHeight(wager) {
    ...
    if (!isCompo) {
      ...
      heigth += categories.length * Constants.RECEIPT_ROW_HEIGHT * 2;
      ...
      if (containsNumberWithExtented) {
        heigth += Constants.RECEIPT_ROW_HEIGHT;
      }
      ...
      heigth = heigth + rows * Constants.RECEIPT_ROW_HEIGHT + Constants.RECEIPT_ROW_HEIGHT;
    }
    ...
    heigth += markerRows * Constants.RECEIPT_ROW_HEIGHT;
  ...
  calculateFireblazeTemplateHeight(wager) {
    ...
    heigth += rows * Constants.RECEIPT_ROW_HEIGHT;
  ...
  getDrawDateTime(timestamp) {
    return this.isL10Version ? moment(timestamp) : moment.unix(timestamp);
  }
}
