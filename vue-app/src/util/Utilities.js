...
export default {
  ...
  /**
   * Check if the serialNumber provided is L5 version
   *
   * @param serialNumber
   * @param lengthType
   * @returns {Boolean}
   */
  isL5Version(serialNumber, lengthType) {
    switch (lengthType) {
      case 0:
        return serialNumber && serialNumber.length === Constants.SERIAL_NUMBER_LENGTH.L5;
      case 1:
        return serialNumber && serialNumber.length === Constants.SERIAL_NUMBER_LENGTH.L5_FULL;
      default:
        return false;
    }
  },
  ...
  /**
   * Get L5 final barcode value without prefix 0 value
   *
   * @param barcode
   * @returns {String}
   */
  setupL5Barcode(barcode) {
    return barcode[0] === "0" && this.isL5Version(barcode, 1) ? barcode.substring(1) : barcode;
  },
  ...
  extractFromL10SerialNumber(inputSerialNumber)
  {
    return {
      ...
      iSecureFirst: inputSerialNumber.toString().substring(
        Constants.SERIAL_NUMBER_LENGTH.L10,
        Constants.SERIAL_NUMBER_LENGTH.L10 + Constants.ISECURE_LENGTH
      ),
      iSecureSecond: inputSerialNumber.toString().substring(
        Constants.SERIAL_NUMBER_LENGTH.L10 + Constants.ISECURE_LENGTH,
  ...
  serialNumberFromBarcode(barcode) {
    let serialNumber = this.setupL5Barcode(barcode);
    if (this.isL10Version(barcode, 1)) {
      serialNumber = this.extractFromL10SerialNumber(barcode).serialNumber;
    }
    return serialNumber;
  },
  ...
  isOddEvenDraw(betType) {
    return [Constants.ILOT_GAMETYPES.ODD, Constants.ILOT_GAMETYPES.EVEN, Constants.ILOT_GAMETYPES.DRAW].includes(betType);
  },
  /**
