...
import SessionStoreModuleTypes from '../SessionStoreModule/types';
...
const getters = {
  ...
  [types.getters.IS_IPC_RENDERER_ENABLED]: ({ config }) => config.IPC_RENDERER_ENABLED,
  [types.getters.IS_DEVELOPMENT_MODE]: ({ config }) => config.NODE_ENV === 'development',
  ...
  [types.getters.GET_FIREBLAZE_COLUMN_PRICE]: ({ config }) => config.FIREBLAZE.COLUMN_PRICE,
  ...
  [types.getters.GET_ASSET_URL]: (state, getters, rootState) => (type, key) => {
    ...
    if (typeof assetFilename !== 'string') {
      ...
      return type === 'video' ? FALLBACK_VIDEO : '';
    }
  ...
  [types.getters.IS_DIGITAL_ASSISTANT_ENABLED]: (state) => {
    return state.config?.DIGITAL_ASSISTANT?.DIGITAL_ASSISTANT_ENABLED;
  },
  [types.getters.GET_BRAND_NAME]: ({ config }) => config.BRAND_NAME || Constants.BRAND_NAMES.OPAP,
};
