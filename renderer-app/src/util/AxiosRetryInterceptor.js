...
const shouldRetry = ({ retryAttempts, maxRetries }) =>
  maxRetries === RETRY_DEFAULTS.INFINITE_MAX_RETRIES || retryAttempts < maxRetries;
...
const handleRequest = async ({ firstRequestDelay, retryDelay, maxRetries }, config) => {
  ...
  config.retryConfig = {
    ...
    retryDelay,
    maxRetries,
    retryAttempts: RETRY_DEFAULTS.RETRY_ATTEMPTS,
    ...retryConfig,
  };
...
const checkResponseData = ({ responseRetryCondition, response }) =>
  responseRetryCondition ? responseRetryCondition(response) : response;

const handleResponse = async ({ axiosInstance, responseRetryCondition, defaultResponse }, response) => {
  const {
    retryConfig: { retryAttempts, maxRetries },
  } = response.config;
  const responseData = checkResponseData({ responseRetryCondition, response });
  return (
    responseData ||
    (shouldRetry({ retryAttempts, maxRetries }) && (await retryRequest({ axiosInstance, config: response.config }))) ||
    defaultResponse
  );
};
...
const setRequestInterceptor = ({ axiosInstance, importedRetryConfig }) => {
  axiosInstance.interceptors.request.use((config) => handleRequest(importedRetryConfig, config));
};

const setResponseInterceptor = ({ axiosInstance, responseRetryCondition, defaultResponse }) => {
  axiosInstance.interceptors.response.use(
    (response) => handleResponse({ axiosInstance, responseRetryCondition, defaultResponse }, response),
    (error) => handleError({ axiosInstance, defaultResponse }, error)
  );
};
...
const addRetryInterceptor = ({
  ...
  setInterceptors({
    axiosInstance,
    importedRetryConfig: { firstRequestDelay, retryDelay, maxRetries },
    responseRetryCondition,
    defaultResponse,
  });
